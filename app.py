...
if st.session_state.phase == 'input':
    ...
    for i in range(st.session_state.revealed_count):
        st.image(os.path.join(pic_p, st.session_state.drawn_cards[i][0]), width=450)
        st.markdown("---")
    ...
    if st.session_state.revealed_count < st.session_state.card_count:
        if st.button(f"✨ 揭晓第 {st.session_state.revealed_count + 1} 张牌"):
            ...
            reply = get_ai_response(f"新翻开第{st.session_state.revealed_count}张：{st.session_state.drawn_cards[st.session_state.revealed_count-1][0]}。继续引导。")
            st.session_state.messages.append({"role": "assistant", "content": reply})
            st.rerun()
    ...
    if st.session_state.messages:
        st.markdown("---")
        report_md = f"# 🧩 OH卡潜意识探索报告\n\n**困惑：** {st.session_state.user_issue}\n**牌阵：** {st.session_state.selected_spread}\n\n---\n\n"
        for m in st.session_state.messages:
            role = "引导师" if m["role"]=="assistant" else "访客"
            report_md += f"**{role}**：{m['content']}\n\n"
        
        st.download_button("💾 下载本次记录 (Markdown)", data=report_md, file_name="OH卡记录.md")