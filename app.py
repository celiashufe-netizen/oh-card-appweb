...
# 2. 🔐 核心：自动读取云端保险柜里的 Key
# 它会自动寻找你刚才在 Streamlit Secrets 里填的 GEMINI_API_KEY
if "GEMINI_API_KEY" in st.secrets:
    genai.configure(api_key=st.secrets["GEMINI_API_KEY"], transport='rest')

# 3. 文档加载
def load_docs():
    sop, spreads_text, names = "你是一位专业OH卡引导师，请通过开放式提问引导用户。", "默认库", ["单卡盲抽"]
    try:
...
def get_ai_response(prompt_text):
    ...
    return "⚠️ 引导师连接失败，请检查 Streamlit Secrets 里的 Key 是否正确。"
...
if st.session_state.phase == 'input':
    st.info("👈 欢迎，Celia。请在左侧输入你的困惑，即可开启探索。")
    ...
    selected_spread = st.selectbox("确认使用的牌阵", spread_options)
    if st.button("🎴 开始发牌"):
    ...
    prompt = f"牌阵【{st.session_state.selected_spread}】需几张牌？只回数字。参考：{spreads_content}"
    ...
    for i in range(st.session_state.revealed_count):
        ...
        st.image(os.path.join(pic_p, img_f), width=450, caption=f"卡位 {i+1}")
        st.markdown("---")
    ...
    if st.session_state.revealed_count < st.session_state.card_count:
        if st.button(f"✨ 揭晓第 {st.session_state.revealed_count + 1} 张牌"):
            st.session_state.revealed_count += 1
            reply = get_ai_response(f"翻开了第{st.session_state.revealed_count}张：{st.session_state.drawn_cards[st.session_state.revealed_count-1][0]}。继续引导。")
            st.session_state.messages.append({"role": "assistant", "content": reply})
            st.rerun()
